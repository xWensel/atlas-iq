...
(function (A) {
  ...
  function adventure() {
    ...
    const decks = Object.keys(D).map(id => {
      ...
      const kit = [...d.tools.map(t => A.icon(A.ADV.TOOLS[t].ico, "kit")), ...d.perks.map(p => A.icon(p, "kit"))].join("");
    ...
    let stakes = ""; for (let i = 0; i <= 5; i++) stakes += `<button class="stake${advSel.asc === i ? " sel" : ""}" data-asc="${i}" ${i > adv.asc ? "disabled" : ""}>${A.icon(STAKE_CHIP[i])}<b>${i}</b></button>`;
  ...
  function profile() {
    ...
    const cell = (l, v, ico) => `<div class="pf-cell">${A.icon(ico)}<span>${l}</span><b>${v}</b></div>`;
